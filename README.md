...
This is a mean-reverting process on $(0, 1)$, analogous to the Feller condition for CIR. Parameter restrictions (analogous to $2\lambda min(\eta, 1-\eta) > \zeta^2$) ensure $H_t$ stays strictly inside $(0, 1)$, so that the kernel $K$ remains well-defined at all times. The ergodic distribution is a **Beta distribution** $(\H_{\infty} ~ \mathcal{N}( \dfrac{2\lambda\eta}{\zeta^{2}},\dfrac{2\lambda(1-\eta)}{\zeta^{2}} ))$, making the parametrisation highly interpretable: $\eta$ controls the long-run average roughness, $\lambda$ its mean-reversion speed, and $\zeta$ the variability of the Hurst exponent.
